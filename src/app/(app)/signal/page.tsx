...
export default function SignalPage() {
  ...
  // The rail is only meaningful against a live ticker: a bundled file is one
  // fixed series, and a board of quotes has nothing to say about it.
  const rail = !dataset;

  return (
    <main className="mx-auto max-w-6xl px-6 py-16">
      {/* The reading on the left, the board on the right — and on a narrow
          screen the columns collapse in DOM order, so the rail lands *below*
          the verdict rather than pushing it off the first screen. That was the
          objection to a sidebar when this was one column; ordering answers it. */}
      <div
        className={`grid gap-8 ${rail ? "lg:grid-cols-[minmax(0,1fr)_20rem]" : ""}`}
      >
